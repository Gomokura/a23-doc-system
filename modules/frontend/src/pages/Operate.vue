...
// 当前预览对应的指令文本（用于检测预览后指令是否被改过）
const previewInstruction = ref('')
// 从预览结果提取操作类型
const executePreviewOpType = computed(() => previewResult.value?.parsed_operation?.type as string || '')
...
async function handlePreview() {
  ...
  try {
    ...
    previewInstruction.value = instruction.value.trim()
...
async function handleExecute() {
  ...
  // 高风险操作必须先预览
  const opType = executePreviewOpType.value
    ...
    <div class="grid grid-cols-12 gap-4">
      ...
      <section class="col-span-8 bg-white border border-border rounded-lg p-4 space-y-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <button @click="handleExecute" :disabled="!canSubmit || busy" class="px-4 py-2 text-white font-medium rounded-md disabled:opacity-50" :class="HIGH_RISK_OPS.has(executePreviewOpType) ? 'bg-orange-500 hover:bg-orange-600' : 'bg-accent hover:opacity-90'">{{ loadingExecute ? '执行中...' : '执行' }}</button>
        ...
        <div v-if="HIGH_RISK_OPS.has(executePreviewOpType)" class="rounded-lg border border-orange-200 bg-orange-50 p-3 text-xs text-orange-800">
          <strong>高风险操作：</strong>当前解析为 <code>{{ executePreviewOpType }}</code>，请先确认预览结果的段落位置正确后再执行。
        </div>
