...
const handleReParse = async (record: UploadHistory) => {
  try {
    const res = await fetch('/api/parse', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ file_id: record.file_id })
    })
    const data = await res.json()
    if (!res.ok) throw new Error(data.detail || '提交失败')
    ElMessage.success(`解析任务已提交，请等待完成`)
    // 刷新状态
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-1.5">
                  <button
                    ...
                    class="px-2 py-1 text-xs bg-accent/10 text-accent rounded hover:bg-accent/20 font-medium"
                  >
                    解析
                  </button>
                  <button
