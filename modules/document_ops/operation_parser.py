...
class OperationParser:
    ...
    def _extract_parameters_by_rules(self, instruction: str, operation_type: str) -> Dict:
        ...
        # 提取段落/位置信息 — 优先匹配"最后/末尾/结尾"
        if any(kw in instruction for kw in ('最后', '末尾', '结尾', '最末')):
            params['position'] = '最后一段'
        elif any(kw in instruction for kw in ('开头', '第一段', '第一节', '第一条')):
            params['position'] = '第一段'
        else:
...
- calculate: 数据计算（如"计算A列总和"）

## 通用操作
- convert_format: 格式转换
- unknown: 无法识别的操作

【重要】parameters 字段格式规范：
对于涉及段落位置的操作（edit_paragraph / format_paragraph / add_paragraph / delete_paragraph），
parameters 中必须包含 "position" 字段，且只能使用以下标准值：
  - "最后一段"  →  用户说"最后"、"末尾"、"结尾"、"最末"时
  - "第一段"    →  用户说"开头"、"第一段"、"第一节"时
  - "第2段"、"第3段" … （具体第几段，用阿拉伯数字）

示例：
  - "删除最后一段"   → {{"operation_type":"delete_paragraph","confidence":0.95,"parameters":{{"position":"最后一段"}},"reasoning":"用户要删除最后一段"}}
  - "删除第二段"     → {{"operation_type":"delete_paragraph","confidence":0.95,"parameters":{{"position":"第2段"}},"reasoning":"用户要删除第二段"}}
  - "把第三段加粗"   → {{"operation_type":"format_paragraph","confidence":0.95,"parameters":{{"position":"第3段","bold":true}},"reasoning":"格式化第三段"}}
  - "在开头添加一段" → {{"operation_type":"add_paragraph","confidence":0.9,"parameters":{{"position":"第一段","content":""}},"reasoning":"在开头插入段落"}}
