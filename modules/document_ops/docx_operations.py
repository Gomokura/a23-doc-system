...
try:
    ...
    # ── Monkey-patch：让 python-docx 跳过 ZIP 中缺失的可选条目 ──────────────
    # 某些由 WPS / 旧版 Word 生成的 docx 缺少 footnotes.xml 等可选文件，
    # python-docx 会在底层 _ZipPkgReader.blob_for 里抛 KeyError 导致 500。
    # 覆写该方法，遇到缺失条目时返回空字节串而非崩溃。
    _original_blob_for = _phys_pkg._ZipPkgReader.blob_for

    def _safe_blob_for(self, pack_uri):
        try:
            return _original_blob_for(self, pack_uri)
        except KeyError:
            logger.warning(f"[docx] ZIP 中缺少可选条目: {pack_uri}，已跳过")
            return b""

    _phys_pkg._ZipPkgReader.blob_for = _safe_blob_for
    # ──────────────────────────────────────────────────────────────────────────
...
class WordDocumentOperations:
    ...
    def __init__(self, file_path: str):
        ...
        self.doc = Document(file_path)  # 缺失 XML 条目已由模块级 patch 处理
